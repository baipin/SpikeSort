...
The notebook loads baseline templates of shape \texttt{(267, 61, 383)} and baseline spike labels. The compressed/reconstructed inputs are derived from the upstream preprocessed signal saved in \texttt{whitened\_data.npy}, where the signal has already undergone \textbf{CAR, bandpass filtering, and spatial whitening} in that order. Reconstructed data is then memory-mapped and reshaped to samples $\times$ channels (reported as \texttt{(1,350,000, 383)}).
...
\begin{enumerate}
  \item DCT per channel on the upstream CAR-corrected, bandpass-filtered, and whitened signal,
